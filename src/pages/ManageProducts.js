...
export default function ManageProducts() {
    ...
    const loadProducts = async () => {
        const result = await fetch("http://8.211.51.110:8080/api/products");
    ...
    const loadCategories = async () => {
        try {
            const result = await fetch("http://8.211.51.110:8080/api/categories");
            const data = await result.json();
            setCategories(Array.isArray(data) ? data : []); // Обеспечиваем, что это массив
        } catch (error) {
    ...
    const deleteProduct = async (id) => {
        await fetch(`http://8.211.51.110:8080/api/products/${id}`, {
            method: 'DELETE',
        });
        loadProducts();
    };
    ...
    const addProduct = async (e) => {
        ...
        try {
            const token = 'your-auth-token'; // Если используете авторизацию, передайте токен

            const response = await fetch("http://8.211.51.110:8080/api/products", {
                method: "POST",
                headers: {
            ...
            if (response.ok) {
                ...
                console.error("Произошла ошибка при добавлении товара!");
            }
        } catch (error) {
